				...
				<u-form-item label="标签">
					<view>
						...
						<view class="Flex-fs addressTag">
							...
							<input placeholder="请输入标签(不超过5个字)" placeholder-style="font-size:24rpx"  maxlength="5" v-model="tag" @input = "onKeyInput" v-show="editDefinedTagShow"/>
						</view>
					</view>
				</u-form-item>
			...
			getConsigneeInfo(id){
				this.$memberApi.getConsigneeInfo(id).then(res => {
					if(res && res.result && res.code === 100){
						...
						this.formdata.region = res.result.province + '-' + res.result.city + '-' + res.result.district;
...
<style lang="scss" scoped>
	...
	.user-setting-addressNew{ padding-top:$uni-padding-30;
		...
		.addressTag1{width: 100%;
			text{display: inline-block; background:rgba(253, 250, 255, 0.9); font-size:$uni-font-size-sm;  color:$uni-text-color-them;  width: 50rpx; }
			}
	}
</style>
