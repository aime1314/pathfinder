<template>
	<view class="user-setting-addressList">
		<view  class="Flex-box" v-for="(item, index) in consigneeInfoList" :key="index">
			...
			<view  class="Flex-sb addsetMsg">
				<view>
					<!-- <text class="font_family icon-xuanzhong icon" v-if="item.defaultAddress == 0"></text>
					<text class="font_family icon-weixuanzhong icon2" v-else></text>
					默认地址 -->
				</view>
				<view v-show="!backUrl" @tap="goUpdateAddress(item.id)"><text class="icon font_family icon-bianji"></text>编辑</view>
				<view v-show="backUrl" @tap="goBack(item.id)"><text class="icon font_family icon-bianji"></text>选择</view>
			</view>
		</view>
...
<style lang="scss" scoped>
	.user-setting-addressList{ padding-top:$uni-padding-30;
		...
		.addressMsg{padding-bottom: $uni-padding-20; align-items:initial; align-content: initial; 
